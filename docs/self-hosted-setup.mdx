...
description: Deploy the GEOK app on your own VPS with nginx, SSL, and provider auth upload: step by step with copy-paste commands.
---
...
bash <(curl -fsSL https://raw.githubusercontent.com/evilstar9527/geok/main/scripts/setup-vps.sh)
```
...
When the script prompts for `Your domain for the app`, enter the hostname you want to serve GEOK on, such as `app.yourdomain.com`. The script configures nginx and requests the TLS certificate for that exact domain.

Or clone first and run locally:

```bash
git clone https://github.com/evilstar9527/geok /tmp/oneglanse
...
This guide deploys GEOK on a fresh Ubuntu 22.04 / 24.04 VPS. At the end you'll have:

- GEOK running behind nginx with HTTPS
...
- GEOK collects responses from real provider UIs, not official model APIs
...
```bash
git clone https://github.com/evilstar9527/geok /home/$USER/oneglanse
...
- GEOK collects from authenticated web UIs, so network reputation matters
...
You should see containers for `web`, `agent-worker`, `analysis-worker`, `postgres`, `clickhouse`, and `redis`.
